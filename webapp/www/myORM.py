...
#定义ORM，主要是创建ModelMetaclass类
#Model只是一个基类，必须要通过metaclass类将子类的User信息读取出来
...
class ModelMetaclass(type):
	...
	def __new__(cls, name, bases, attrs):
		...
		#构造默认的Select, Insert, Update, Delte语句
...
class Model(dict, metaclass = ModelMetaclass):
	...
	#往Model类添加class方法，让所有的子类调用class方法
	@classmethod
	@asyncio.coroutine
	...
	def findNumber(cls, selectField, where = None, args = None):
		...
		sql = ['select %s _num_ from %s' % (selectField, cls.__table_)]
		if where:
			sql.append('where')
			sql.append(where)
